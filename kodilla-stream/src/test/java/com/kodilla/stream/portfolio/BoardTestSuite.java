...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList>inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progres"));
            double averageOfDays = project.getTaskLists().stream()
                    .filter(inProgressTasks::contains)
                    .flatMap(x->x.getTasks().stream())
                    .mapToDouble(x1-> x1.getCreated().until(LocalDate.now(), ChronoUnit.DAYS))
                    .average()
                    .getAsDouble();


Assert.assertEquals(14.166,averageOfDays,0.001);
